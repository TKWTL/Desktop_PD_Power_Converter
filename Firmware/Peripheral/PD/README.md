...
| `pd_port.c` | PHY/CC | `USBPD`/CC/GPIO/RCC/NVIC 寄存器;USBPD 专用 DMA 收发;**异步 TX 引擎**(`PD_Port_StartTx()` + USBPD IRQ + deadline/重发);ISR 内自动 GoodCRC;RX 双缓冲(mailbox);Hard Reset 物理发送 |
...
1. 帧一旦交给 USBPD 专用 DMA,**只能**由 USBPD 中断链(TX_END → RX 转向 → GoodCRC)完成;
   禁止在任何层次重新引入忙等/轮询(旧的 `PD_Port_TransactSOP()` 已删除);
2. 帧正在发送时(自动 GoodCRC 或前台 TX)不得执行 `PD_ALL_CLR` 或重新启动 RX
   ——`PD_Port_RxStart()` 已按 PHY 状态自我门控;
3. TX 完成/失败由 `PD_Port_Service()`(deadline)与 `PD_TxCompletion_Proc()`(`PD_TxJob` 上下文)统一处理;
